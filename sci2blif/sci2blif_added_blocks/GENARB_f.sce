...
if (blk_name{bl} =='GENARB_f') then
    ...
    for ii = 1:ga_idx2
        ...
        arb_gen(1,1){1,ii}= strtod(dac_loc(loc_num,dac_loc_idx){2})
        mputl('net'+ string(blk(blk_objs(bl),2+numofip)) + "_"+ string(ii) +' ' + dac_loc(loc_num,dac_loc_idx){1},fd_io);
    end
    mclose(fd_io);
    genarb_dac = arb_gen(1,1){1,:};
