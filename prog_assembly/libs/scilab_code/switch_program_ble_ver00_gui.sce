...
switch_list = fscanfMat(hid_dir+'/switch_list_ble');temp_size= size(switch_list); n=temp_size(1,1);

// Check the size of switches and divide files
n_temp = n;
maximum_swc_number = 170;
...
fd = mopen(hid_dir+'/switch_info_ble','r'); clear switch_info_data; switch_info_data = [1:3];
...
unix_g('sudo mv switch_info_* ' + hid_dir);
...
while j <= number_of_switchfiles,
    ...
    if (b1==0) then
        [a2,b2]=unix_g("sudo tclsh ~/rasp30/prog_assembly/libs/tcl/program.tcl -speed 115200 "+hid_dir+"/switch_program_ble.elf");
...
fprintfMat("Switch_program_ble_Current"+fname+".data", output_file, "%5.15f");
...
unix_g('sudo mv *.hex Switch_program_ble_Current' + fname + '.data unprogrammed_switches ' + hid_dir);
