			...
			for subblock in subblocks:                
				...
				if (sub_inst[:-3] == 'counter8') & (sub_outputs != 'open'):
					...
					if nsb.name =="vmm4x4[0]":
						sub_outputs=subblock.findall("outputs")[0].findall("port")[0].text.split()
					if nsb.name in ["mite2[0]", "signalmult[0]","hhneuron[0]",'ramp_fe[0]','hhn_debug[0]','SubbandArray[0]']:
						sub_outputs=subblock.findall("outputs")[0].findall("port")[0].text.split()
					...
					if outputs[i] != 'open':
						...
						if from_sub_name ==['SR4[0]']:
							...
							try:
								...
								pdb.set_trace()
						elif from_sub_name in ['mite2[0]','signalmult[0]','hhneuron[0]','ramp_fe[0]','hhn_debug[0]','SubbandArray[0]']:
							try:
